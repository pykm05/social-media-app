...
-- You won't be able to log into these accounts since our backend hashes passwords. SQL does not natively support hashing so by inserting just a raw text password, when you try to log in,
-- it'll compare the hashed password against this unhashed password and it won't work. 
...
-- Do keep in mind that these ones use foreign keys that require the post_id which is autogenerated so depending on the time that you enter these sql commands,
-- the votes and comments might not map. In our database, however, it does work.
